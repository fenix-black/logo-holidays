...
/**
 * Creates a blank, base64-encoded PNG image.
 * @param width The width of the canvas.
 * @param height The height of the canvas.
 * @returns An object with the base64 string and mimeType.
 */
const createBlankImageB64 = (width: number, height: number): { b64: string; mimeType: string } => {
...
export const generateHolidayImage = async (logoB64: string, logoMimeType: string, holiday: Holiday, country: string, logoAnalysis: string, style: string): Promise<{b64: string, mimeType: string}> => {
    try {
...
5.  **CANVAS CONFORMITY**: The generated image MUST exactly fit the dimensions of the provided blank canvas (the second image). Use the blank canvas as your size and aspect ratio template - the output image must match its exact dimensions and 16:9 aspect ratio.`;
        ...
        const response = await ai.models.generateContent({
            ...
            contents: {
                parts: [
                    { inlineData: { data: logoB64, mimeType: logoMimeType } },
                    // Add the blank canvas as the last image to enforce its aspect ratio
...
export const generateVideoPromptJson = async (holiday: Holiday, country: string, style: string): Promise<string> => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: "gemini-2.5-pro",
            contents: `Create a detailed, creative JSON prompt for the Veo video model to animate an image celebrating '${holiday.name_en}' in ${country}. All scenes should be composed for a 16:9 aspect ratio. The holiday is about: "${holiday.description_en}".
${styleInstruction}
